...
try {
  ...
  const access = (await db.query("SELECT has_table_privilege('anon','public.community_messages','SELECT') anon_read,has_table_privilege('authenticated','public.community_messages','INSERT') member_insert,has_table_privilege('authenticated','operations_private.community_authors','SELECT') member_authors,has_function_privilege('anon','public.community_send(text,uuid)','EXECUTE') anon_send")).rows[0];
