...
try {
  ...
  const send = (await db.query("SELECT coalesce(to_regprocedure('public.community_send(text,uuid,boolean,uuid)'),to_regprocedure('public.community_send(text,uuid,boolean)'))::text signature")).rows[0].signature;
  const access = (await db.query("SELECT has_function_privilege('anon',$1,'EXECUTE') anon_send,has_function_privilege('authenticated',$1,'EXECUTE') member_send,has_table_privilege('authenticated','operations_private.community_authors','SELECT') private_read,has_table_privilege('authenticated','public.community_messages','INSERT') direct_insert",[send])).rows[0];
  assert.deepEqual(access, {anon_send:false,member_send:true,private_read:false,direct_insert:false});
  assert.equal((await db.query("SELECT pronargdefaults FROM pg_proc WHERE oid=$1::regprocedure",[send])).rows[0].pronargdefaults, send.includes('boolean,uuid') ? 2 : 1);
