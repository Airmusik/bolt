...
test('installable site has manifest and safe navigation fallback',()=>{
  ...
  assert.ok(manifest.icons.some((icon:{src:string;sizes:string})=>icon.src==='/app-icon-192.png?v=5'&&icon.sizes==='192x192'));
  assert.ok(manifest.icons.some((icon:{src:string;sizes:string;purpose:string})=>icon.src==='/app-icon-512.png?v=5'&&icon.sizes==='512x512'&&icon.purpose.includes('maskable')));
  assert.match(readFileSync('public/app-icon.svg','utf8'),/aria-label="11Drive steering wheel"/);
  const worker=readFileSync('public/sw.js','utf8');assert.match(worker,/request\.mode==='navigate'/);
  assert.match(worker,/11drive-shell-v6/);
  ...
  assert.match(app,/const installedAppLaunch =/);
  assert.match(app,/if \(installedAppLaunch\) return true/);
  assert.match(app,/installedAppLaunch \|\| settings\.launch_intro_enabled === 'true'/);
  ...
  assert.match(app,/!showLaunchIntro && !installedAppLaunch && <span aria-hidden="true"/);
...
test('member tools include profile, report, connection, and calendar states',()=>{
  ...
  assert.match(checklist,/platform_history_submitted\|\|!driverNeedsApproval\(profile\)/);
