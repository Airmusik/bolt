...
test('reject country-only locations without blocking specific Kenyan towns', () => {
  for (const value of ['Kenya', ' KENYA ', 'Kenya.', 'Kenya, Kenya']) assert.equal(isBroadNairobi(value), true, value);
  for (const value of ['Kisumu, Kenya', 'Karen, Nairobi, Kenya', '']) assert.equal(isBroadNairobi(value), false, value);
});
