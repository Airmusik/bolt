...
export function ProfileCompletionChecklist({profile}:{profile:Profile}){
 ...
 const finishPath=!photoComplete?'/settings?from=profile-health#profile-details':profile.role==='driver'&&(!aboutComplete||!driverDetailsComplete)?'/onboarding?from=profile-health#about-you':profile.role==='driver'&&!historyComplete?'/onboarding?from=profile-health#platform-history':'/settings?from=profile-health#profile-details';
 if(percent===100)return null;
 return <section className="dashboard-panel"><div className="flex items-center justify-between gap-3"><div><p className="flex items-center gap-2 font-semibold text-ink-900"><UserRoundCheck className="h-4 w-4"/>Complete your profile</p><p className="mt-1 text-xs text-ink-500">Complete profiles are easier for other members to understand.</p></div><strong className="text-sm text-brand-700">{percent}%</strong></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-ink-100"><div className="h-full rounded-full bg-emerald-500 transition-all" style={{width:`${percent}%`}}/></div><ul className="mt-3 grid gap-2 sm:grid-cols-2">{items.map(([label,complete])=><li key={label} className={`flex items-center gap-2 text-xs ${complete?'text-emerald-700':'text-ink-600'}`}>{complete?<CheckCircle2 className="h-4 w-4"/>:<Circle className="h-4 w-4"/>}{label}</li>)}</ul><Link to={finishPath} className="btn-secondary mt-4 text-xs">Finish profile</Link></section>
}
