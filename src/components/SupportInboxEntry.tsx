...
export function SupportInboxEntry({ search }: { search: string }) {
  const { user } = useAuth();
  ...
  return <Link to={supportInboxPath(latest ? new URLSearchParams({ message: latest.id }) : '')} className="mx-2 mt-2 flex items-center gap-3 rounded-xl border border-ink-100 p-3 text-left hover:bg-ink-50">
    <SiteLogo size={46} />
