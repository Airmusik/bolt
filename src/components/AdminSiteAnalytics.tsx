...
export function AdminSiteAnalytics() {
  ...
  const cards = report ? [
    { label: 'Page views', value: report.views, note: 'Measured page loads and navigation', icon: Eye, color: 'text-ink-600 bg-ink-50' },
    { label: 'Browser sessions', value: report.sessions, note: 'Distinct browser tabs, not unique people', icon: Globe2, color: 'text-ink-600 bg-ink-50' },
    { label: 'New signups', value: report.signups, note: 'Non-admin accounts created', icon: UserPlus, color: 'text-ink-600 bg-ink-50' },
    { label: 'Recently signed in', value: report.logged_in, note: 'Accounts whose latest login falls in this range', icon: LogIn, color: 'text-ink-600 bg-ink-50' },
    { label: 'Active members', value: report.active_members, note: 'Signed-in members with measured activity', icon: Users, color: 'text-ink-600 bg-ink-50' },
    { label: 'Online now', value: report.online, note: 'Measured members active in the last 5 minutes', icon: Activity, color: 'text-ink-600 bg-ink-50' },
    { label: 'New listings', value: report.listings, note: 'Listings created, including pending approval', icon: Car, color: 'text-ink-600 bg-ink-50' },
    { label: 'Connection requests', value: report.connections, note: 'Requests created in the selected range', icon: Link2, color: 'text-ink-600 bg-ink-50' },
  ] : [];
  ...
  const explanations: Record<string, string> = {
    ...
    'Active members': 'Distinct signed-in accounts with analytics enabled and visited during your selected dates. A member visiting many times still counts once.',
    'Online now': 'Signed-in members with analytics enabled and had a visible page active in the last five minutes. This number ignores the date filter and may include someone who just left.',
  ...
  return <section className="space-y-6" aria-label="Site analytics">
    <div className="analytics-banner rounded-2xl border border-ink-200 bg-ink-50 p-6 text-ink-900">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-ink-600">11Drive insights</p><h2 className="mt-1 text-3xl font-bold">See how your community grows</h2><p className="mt-2 text-sm text-ink-600">Traffic, countries and marketplace activity—explained simply.</p></div><span className="flex items-center gap-2 text-xs"><span className="h-2 w-2 rounded-full bg-emerald-300" />{busy ? 'Updating…' : auto ? 'Refreshes every 60 seconds' : 'Manual refresh'}</span></div>
    </div>
...
<button className="btn-secondary text-xs" disabled={busy || !report || !valid || !!error} onClick={exportCsv}><Download className="h-4 w-4" />Export CSV</button>
    ...
    {report && valid && <>
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{cards.map((card, index) => <button type="button" key={card.label} aria-expanded={selected === card.label} aria-controls="analytics-card-detail" onClick={() => { setSelected(selected === card.label ? null : card.label); if (card.label === 'Page views') setMetric('views'); if (card.label === 'New signups') setMetric('signups'); }} className={`analytics-metric rounded-2xl border bg-white p-4 text-left shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent-500 dark:bg-ink-100 ${selected === card.label ? 'border-accent-500 ring-2 ring-accent-500/20' : 'border-ink-200 hover:border-ink-400'}`}><span className={`analytics-metric-icon inline-flex rounded-xl p-2 ${card.color}`} style={{ animationDelay: `${index * .4}s` }}><card.icon className="h-5 w-5" /></span><p className="mt-3 text-sm font-semibold">{card.label}</p><p className="mt-1 text-3xl font-bold tabular-nums">{card.value.toLocaleString()}</p><p className="mt-2 text-xs text-ink-500">{card.note}</p><span className="mt-3 block text-xs font-semibold text-ink-700">{selected === card.label ? 'Hide details ↑' : 'View details ↓'}</span></button>)}</div>
      <div id="analytics-card-detail" aria-live="polite">{detail && <div className="rounded-2xl border border-ink-200 bg-ink-50 p-5">
      ...
      <div className="rounded-2xl border border-ink-200 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="flex items-center gap-2 font-bold"><TrendingUp className="h-5 w-5 text-ink-600" />Daily growth</h3><select aria-label="Chart metric" className="input w-auto" value={metric} onChange={e => setMetric(e.target.value as 'views' | 'signups')}><option value="views">Page views</option><option value="signups">Signups</option></select></div>
        <p className="mt-2 text-xs text-ink-500">Hover, focus or tap a day to inspect it. Scroll sideways for longer ranges.</p>
        <div className="mt-3 min-h-10 rounded-lg bg-ink-50 p-2 text-sm text-ink-700" role="status">{report.daily.find(d => d.day === hoveredDay) ? report.daily.filter(d => d.day === hoveredDay).map(d => <span key={d.day}><strong>{d.day}</strong> · {d.views.toLocaleString()} page views · {d.signups.toLocaleString()} signups</span>) : 'Move over the graph to see exact figures.'}</div>
        <div className="mt-5 overflow-x-auto"><div className="flex h-48 items-end gap-2 border-b border-ink-200" style={{ minWidth: report.daily.length * 34 }}>{report.daily.map(d => <button type="button" key={d.day} onMouseEnter={() => setHoveredDay(d.day)} onFocus={() => setHoveredDay(d.day)} onClick={() => setHoveredDay(d.day)} aria-label={`${d.day}: ${d.views} page views, ${d.signups} signups`} aria-pressed={hoveredDay === d.day} className={`flex h-full min-w-6 flex-1 flex-col items-center justify-end rounded-t-md text-center focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent-500 ${hoveredDay === d.day ? 'bg-ink-100' : 'hover:bg-ink-50'}`}><span className="text-[10px] text-ink-500">{d[metric]}</span><span className={`mx-auto block w-full max-w-10 rounded-t-md motion-safe:transition-all motion-safe:duration-200 ${hoveredDay === d.day ? 'bg-accent-600' : 'bg-accent-500'}`} style={{ height: `${Math.max(d[metric] > 0 ? 2 : 0, d[metric] / peak * 75)}%` }} /><span className="mt-2 text-[9px] text-ink-500">{d.day.slice(5)}</span></button>)}</div></div>
      ...
      <div className="grid gap-6 lg:grid-cols-2"><div className="rounded-2xl border border-ink-200 p-5"><h3 className="font-bold">Where visitors connect from</h3><p className="mt-1 text-xs text-ink-500">Approximate network country, not nationality. VPNs may change it.</p><input className="input mt-4 w-full" placeholder="Find a country…" aria-label="Search countries" value={countrySearch} onChange={e => setCountrySearch(e.target.value)} />
        <div className="mt-4 max-h-80 space-y-4 overflow-y-auto">{report.countries.filter(c => countryName(c.country).toLowerCase().includes(countrySearch.toLowerCase())).map(c => <div key={c.country} tabIndex={0} title={`${countryName(c.country)}: ${c.sessions} sessions, ${c.views} views`} aria-label={`${countryName(c.country)}: ${c.sessions} sessions, ${c.views} views`} className="rounded-lg p-2 hover:bg-ink-50 focus:bg-ink-50 focus:outline focus:outline-2 focus:outline-accent-500 dark:hover:bg-ink-100 dark:focus:bg-ink-100"><div className="flex justify-between gap-3 text-sm"><span>{countryName(c.country)}</span><span className="text-ink-500">{c.sessions} sessions · {c.views} views</span></div><div className="mt-2 h-2 rounded-full bg-ink-100"><div className="h-full rounded-full bg-accent-500" style={{ width: `${c.sessions / Math.max(1, ...report.countries.map(x => x.sessions)) * 100}%` }} /></div></div>)}{!report.countries.length && <p className="text-sm text-ink-500">Countries will appear as measured visits arrive.</p>}</div>
      </div><div className="rounded-2xl border border-ink-200 p-5"><h3 className="font-bold">Most-viewed pages</h3><p className="mt-1 text-xs text-ink-500">Page categories only—no private record IDs or search terms.</p><ol className="mt-5 space-y-4">{report.pages.map((p, i) => <li key={p.path} className="flex items-center gap-3 text-sm"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-ink-50 font-bold text-ink-700">{i+1}</span><span className="flex-1 break-all">{p.path === '/' ? 'Home' : p.path}</span><strong>{p.views.toLocaleString()}</strong></li>)}</ol>{!report.pages.length && <p className="mt-4 text-sm text-ink-500">No measured page views yet.</p>}</div></div>
      <details className="rounded-2xl bg-ink-50 p-5 text-sm"><summary className="cursor-pointer font-bold">How to read these numbers</summary><div className="mt-3 space-y-2 text-ink-600"><p>Traffic is an estimate from visitors who have not opted out. Admins and Do Not Track browsers are excluded. A session is a browser tab, so it is not a unique person; returning visits and multiple devices may count separately.</p><p>Online now is independent of the date filter. It counts measured signed-in members whose visible page sent activity within five minutes. Active members are distinct signed-in accounts measured in the selected range.</p><p>Recently signed in counts each account once using its latest successful sign-in time. It is not a historical login-event count: signing in again outside a past range can change that past range's total.</p><p>Signups, listings and connection requests use existing operational records, including records predating traffic tracking. Deleted records are not counted. Traffic is limited to the last 90 days; older visit records are cleaned up when new activity arrives. Country totals can overlap if one session changes network country.</p></div></details>
    </>}
  </section>;
}
