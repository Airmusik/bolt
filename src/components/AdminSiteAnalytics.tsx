  ...
  return <section className="space-y-6" aria-label="Site analytics">
    ...
    {report && valid && <>
      ...
      <div className="rounded-2xl border border-ink-200 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="flex items-center gap-2 font-bold"><TrendingUp className="h-5 w-5 text-emerald-600" />Daily growth</h3><select aria-label="Chart metric" className="input w-auto" value={metric} onChange={e => setMetric(e.target.value as 'views' | 'signups')}><option value="views">Page views</option><option value="signups">Signups</option></select></div>
        ...
        <div className="mt-3 min-h-10 rounded-lg bg-emerald-50 p-2 text-sm text-emerald-900" role="status">{report.daily.find(d => d.day === hoveredDay) ? report.daily.filter(d => d.day === hoveredDay).map(d => <span key={d.day}><strong>{d.day}</strong> · {d.views.toLocaleString()} page views · {d.signups.toLocaleString()} signups</span>) : 'Move over the graph to see exact figures.'}</div>
        <div className="mt-5 overflow-x-auto"><div className="flex h-48 items-end gap-2 border-b border-ink-200" style={{ minWidth: report.daily.length * 34 }}>{report.daily.map(d => <button type="button" key={d.day} onMouseEnter={() => setHoveredDay(d.day)} onFocus={() => setHoveredDay(d.day)} onClick={() => setHoveredDay(d.day)} aria-label={`${d.day}: ${d.views} page views, ${d.signups} signups`} aria-pressed={hoveredDay === d.day} className={`flex h-full min-w-6 flex-1 flex-col items-center justify-end rounded-t-md text-center focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-600 ${hoveredDay === d.day ? 'bg-emerald-100/60' : 'hover:bg-emerald-50'}`}><span className="text-[10px] text-ink-500">{d[metric]}</span><span className={`mx-auto block w-full max-w-10 rounded-t-md bg-gradient-to-t motion-safe:transition-all motion-safe:duration-200 ${hoveredDay === d.day ? 'from-sky-700 to-cyan-400 shadow-md' : 'from-teal-600 to-emerald-300'}`} style={{ height: `${Math.max(d[metric] > 0 ? 2 : 0, d[metric] / peak * 75)}%` }} /><span className="mt-2 text-[9px] text-ink-500">{d.day.slice(5)}</span></button>)}</div></div>
      ...
      <div className="grid gap-6 lg:grid-cols-2"><div className="rounded-2xl border border-ink-200 p-5"><h3 className="font-bold">Where visitors connect from</h3><p className="mt-1 text-xs text-ink-500">Approximate network country, not nationality. VPNs may change it.</p><input className="input mt-4 w-full" placeholder="Find a country…" aria-label="Search countries" value={countrySearch} onChange={e => setCountrySearch(e.target.value)} />
        <div className="mt-4 max-h-80 space-y-4 overflow-y-auto">{report.countries.filter(c => countryName(c.country).toLowerCase().includes(countrySearch.toLowerCase())).map(c => <div key={c.country} tabIndex={0} title={`${countryName(c.country)}: ${c.sessions} sessions, ${c.views} views`} aria-label={`${countryName(c.country)}: ${c.sessions} sessions, ${c.views} views`} className="rounded-lg p-2 hover:bg-sky-50 focus:bg-sky-50 focus:outline focus:outline-2 focus:outline-sky-500 dark:hover:bg-ink-100 dark:focus:bg-ink-100"><div className="flex justify-between gap-3 text-sm"><span>{countryName(c.country)}</span><span className="text-ink-500">{c.sessions} sessions · {c.views} views</span></div><div className="mt-2 h-2 rounded-full bg-ink-100"><div className="h-full rounded-full bg-gradient-to-r from-sky-500 to-violet-500" style={{ width: `${c.sessions / Math.max(1, ...report.countries.map(x => x.sessions)) * 100}%` }} /></div></div>)}{!report.countries.length && <p className="text-sm text-ink-500">Countries will appear as measured visits arrive.</p>}</div>
