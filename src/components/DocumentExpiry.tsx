...
export function DocumentExpiry({ expiresAt }: { expiresAt: string }) {
  ...
  return <div className={cn('mt-2 rounded-xl border p-3 text-xs', nearExpiry ? 'border-amber-200 bg-amber-50 text-amber-900 dark:bg-amber-950/30 dark:text-amber-100' : 'border-ink-200 bg-ink-50 text-ink-700')}>
    <p className="flex items-center gap-2 font-semibold"><Clock3 className="h-4 w-4 shrink-0" />{expiryCountdown(expiresAt, now)}</p>
    <p className="mt-1">Expires {formatDateTime(expiresAt)}</p>
  </div>;
}
