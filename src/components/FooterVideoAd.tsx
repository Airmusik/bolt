...
import { adPageAllowed, safeAdMediaUrl, safeAdUrl, videoAdIsVisible, type AdSettings } from '@/lib/ads';
...
export function FooterVideoAd() {
  ...
  return <div className="mb-6 flex justify-end"><VideoAdCreative key={settings.ads_video_url} settings={settings} onDismiss={() => setDismissed(true)} /></div>;
}
...
export function VideoAdCreative({ settings, onDismiss, preview = false }: { settings: AdSettings; onDismiss?: () => void; preview?: boolean }) {
  ...
  const src = safeAdMediaUrl(settings.ads_video_url, 'video');
  const destination = safeAdUrl(settings.ads_video_destination);
  ...
  return <aside aria-label="Video advertisement" className="w-full max-w-sm overflow-hidden rounded-2xl border border-ink-200 bg-white shadow-sm dark:border-white/15 dark:bg-[#24272e]">
    ...
    <div className="space-y-2 p-3">
      ...
      <p className="break-words text-sm font-semibold text-ink-900">{settings.ads_video_title || 'Your video headline'}</p>
