    ...
    {error ? <p role="alert" className="mt-3 text-sm text-danger">{error}</p> : !loaded ? <p className="mt-3 text-sm">Loading analytics…</p> : <>
      ...
      <div className="mt-3 space-y-3">{visible.map(r => {
        ...
        return <article key={r.id} className="rounded-xl border border-ink-100 p-3">
          ...
          <dl className="mt-3 grid grid-cols-3 gap-2"><div className="rounded-lg border border-ink-100 bg-ink-50 p-2 text-ink-900"><dt className="text-xs text-ink-600">Reach</dt><dd className="text-lg font-bold">{reach.toLocaleString()}</dd></div><div className="rounded-lg border border-ink-100 bg-ink-50 p-2 text-ink-900"><dt className="text-xs text-ink-600">Clicks</dt><dd className="text-lg font-bold">{clicks.toLocaleString()}</dd></div><div className="rounded-lg border border-ink-100 bg-ink-50 p-2 text-ink-900"><dt className="text-xs text-ink-600">Click rate</dt><dd className="text-lg font-bold">{reach ? (clicks / reach * 100).toFixed(1) : '0'}%</dd></div></dl>
