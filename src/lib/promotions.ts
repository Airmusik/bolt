...
export function promotionProgress(request: PromotionRequest, visible = true) {
  ...
  const states = {
    ...
    active: { label: visible ? 'Promotion running' : 'Promotion active · not currently showing', tone: visible ? 'badge-success' : 'badge-warning', help: visible ? 'Your promotion appears in the top promoted section on car or driver searches while eligible. Matching promotions appear first; approval and availability still apply.' : 'The promotion dates remain in effect, but it is not currently eligible to appear. Check availability and approval, or ask support for help.' },
