...
export function useSeo() {
  ...
  useEffect(() => {
    ...
    const title = path === '/' ? settings.seo_home_title : page?.title ?? 'Your account | 11Drive';
    const description = path === '/' ? settings.seo_home_description : page?.description ?? 'Manage your 11Drive account.';
    ...
    if (path === '/') meta('keywords', settings.seo_keywords);
  ...
  }, [pathname, search, settings.seo_home_title, settings.seo_home_description, settings.seo_keywords]);
}
